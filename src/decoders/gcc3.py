...
def GCC_Annihilate(cluster, uc, code, ct, s,i):
	...
	for type in test_code.types:
		...
		cc[type], test_uc, test_code = GCC_One_Color_Simplify(cc[type], test_uc, test_code, type, ct, cntr, s, bad_code)		
	print "CCCCCCC", cc[type]
	cc, test_uc, test_code = GCC_Two_Color_Simplify(cc, test_uc, test_code, ct, cntr, bad_code)
	
	# print test_uc.nodes(data=True)

	if test_uc.nodes() == []:
		# visualization.PlotPlaquette(test_code, "Neutral", i+1)
		print "NEUTRAL!!!!!", cc
		# print pnts
		for pnt in pnts:
			uc.remove_node(pnt)
		...
		# visualization.PlotPlaquette(test_code, "Boundary-Neutral", i+1)
		test_code = boundAnnihilate(cc, test_uc, test_code, ct, cntr, s, bad_code)
		for node in cluster:
...
def boundAnnihilate(cc, uc, code, ct, center, scale, bad_code):
	code = GCC_Boundary_Simplify(cc, uc, code, ct, center, scale, bad_code)[2]
	return code

def isBoundNeutral(cc, code, center, scale):
	# this is why the qudit corrections are failing
...
def GCC_One_Color_Simplify(cc, uc, code, t, ct, cntr, scale, bad_code):
	print cc
	d = code.dimension
	print cntr, "BEFORE"
	if cc == []:
	...
	# print code.Stabilizers[t]
	mid_m = closest_to_point(bad_code.Stabilizers[t],cntr)
	print mid_m, "AFTER"
...
def GCC_Boundary_One_Color_Simplify(m, cc, uc, code, t, ct, cntr, scale,bad_code):
	...
	if any(common.euclidean_dist(ext, cntr) < scale for ext in code.External[t]):
		for ext in code.External[t]:
			if common.euclidean_dist(ext, cntr) < scale:
				...
				cc[t], uc, code = GCC_One_Color_Simplify(cc[t], uc, code, t, ct, cntr, scale,bad_code)		
		...
		if any(ext1 in code.External[t1] for ext1 in code.Dual[t2].neighbors(m)) and any(ext2 in code.External[t2] for ext2 in code.Dual[t1].neighbors(m)):
			...
			for m_GCC in code.Stabilizers[t]:
				if any(ext1 in code.External[t1] for ext1 in code.Dual[t2].neighbors(m_GCC)) and any(ext2 in code.External[t2] for ext2 in code.Dual[t1].neighbors(m_GCC)):
					break
			s, e = cc[t][0], (m_GCC,{'charge':0,'type':t})
			uc.add_node(m_GCC, charge = 0, type = t)
			cc[t].append(e)
			cc[t], uc, code = GCC_One_Color_Transport(s, e, cc[t], uc, code, t, ct)

		for ext1 in code.External[t1]:
			if ext1 in code.Dual[t2].neighbors(m_GCC):
				break
		for ext2 in code.External[t2]:
			if ext2 in code.Dual[t1].neighbors(m_GCC):
				break
		...
		cc, uc, code = GCC_Two_Color_Simplify(cc, uc, code, ct, cntr, bad_code)
	return cc, uc, code 

def GCC_Boundary_Two_Color_Simplify(ints, cc, uc, code, ct, cntr, scale, bad_code):
	# print "HOWDY ROWDY"
	# print ints
	[m0, m1] = ints
	...
	if c0 == c1:
		...
		if any((common.euclidean_dist(ext, cntr) < scale and common.euclidean_dist(ext, cntr) < scale) for ext in code.External[t2]):
			...
			# print "YEEEEHHAAAAAA"
			cc, uc, code = GCC_Boundary_One_Color_Simplify(m0[0], cc, uc, code, t0, ct, cntr, scale, bad_code)
			cc, uc, code = GCC_Boundary_One_Color_Simplify(m1[0], cc, uc, code, t1, ct, cntr, scale, bad_code)
...
# Everything Below this line is the same as in gcc: #
    ...
    def __call__(self, code, ct):
        ...
        while uc.nodes() != []:
        	...
        	for cluster in clusters:
        		code, uc = GCC_Annihilate(cluster, uc, code, ct, (i)*scale,i)
...
def GCC_Two_Color_Simplify(cc, uc, code, ct, cntr, bad_code):
...
def GCC_Boundary_Simplify(cc, uc, code, ct, cntr, scale, bad_code):
	ints = cc['red'] + cc['blue'] + cc['green']

	if len(ints) == 2:
		# print "TWO COLS"
		cc, uc, code = GCC_Boundary_Two_Color_Simplify(ints, cc, uc, code, ct, cntr, scale, bad_code)
	elif len(ints) == 1:
		t, m = ints[0][1]['type'], ints[0][0]
		# print "ONE YALL"
		cc, uc, code = GCC_Boundary_One_Color_Simplify(m, cc, uc, code, t, ct, cntr, scale, bad_code)
